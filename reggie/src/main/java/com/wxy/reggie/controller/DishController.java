...
//        System.out.println(dishDto.toString());
     ...
     * queryWrapper.eq(Dish::getStatus, 1);//只查询在售的商品
     * //添加排序条件
    ...
    public R<List<DishDto>> list(Dish dish) {
        ...
        List<DishDto> dishDtoList = list.stream().map((item) -> {
            ...
            LambdaQueryWrapper<DishFlavor> lambdaQueryWrapper = new LambdaQueryWrapper<>();//根据菜品id查询口味数据
            lambdaQueryWrapper.eq(DishFlavor::getDishId, dishId);
            List<DishFlavor> dishFlavors = dishFlavorService.list(lambdaQueryWrapper);//查询口味数据
            dishDto.setFlavors(dishFlavors);//设置口味数据
            return dishDto;
        }).collect(Collectors.toList());
